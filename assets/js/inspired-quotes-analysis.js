(function () {
  ...
  const PART_COLORS = ["#0d3d1f", "#126b32", "#18A54B", "#5cb87a", "#b8e6c8"];
  const MAX_QUOTES = 26;
  ...
  function renderStats(summary) {
    ...
    const items = [
      { label: "Total quotes", value: String(summary.totalQuotes), tone: "info" },
      { label: "Words highlighted", value: summary.totalWords.toLocaleString("en-US"), tone: "success" },
      { label: "Avg words/quote", value: String(summary.avgWordsPerQuote) },
      { label: "Share of book (est.)", value: summary.pctEstimated + "%", tone: "warning" },
    ];
    items.forEach(function (item) {
      const card = el("div", "iqa-stat iqa-stat--" + (item.tone || "default"));
      card.appendChild(el("div", "iqa-stat__value", item.value));
  ...
  function renderChapterTable(chapters) {
    ...
    chapters.forEach(function (chapter) {
      const tr = el("tr");
      if (chapter.quotes >= 10) tr.className = "iqa-table__highlight";
  ...
  function renderFullOverview(chapters, partCounts, partMeta) {
    ...
    wrap.appendChild(el("p", "iqa-caption", "Bar scaled to maximum (Ch. 10 = 26 quotes)"));
    return wrap;
  }
  ...
  function render(data) {
    const root = document.getElementById("inspired-quotes-analysis");
    if (!root) return;

    const summary = data.summary;
    const fragment = document.createDocumentFragment();
    fragment.appendChild(el("h3", "iqa-heading", "INSPIRED quote analysis"));
    fragment.appendChild(renderStats(summary));
    fragment.appendChild(renderCard("Quotes by book part", renderDonut(data.partCounts, data.partMeta, summary.totalQuotes)));
